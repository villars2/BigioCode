...
NomList={'COE','WAGES','GDP','INV','PNRI','PRI','COFC','CONS','NONDURCO','DURCO','GOV','EXP','M','HOURS','GDPDEF'};  % 'NX',
varlist=[Alist FernaldName_List rinv_list cap_list NomList {'IPMAN','RTTLCONS','RPCESV','RGDP','RINV','RDURINV','RPNRI','RPRI','R_PNRI','R_PRI','RCONS','R_NONDURCO','R_DURCO','RGOV','REXP','RM','RCPH','HOURS','Y_L','GDPDEF'}];  %'RCOFC',

% Linear Differences and Conversion to Real Terms
% (this tests the accuracy of the GDP Deflator)
BDP_logdiff;

% Growth Rates and trends
varlist=[Alist FernaldName_List rinv_list cap_list NomList {'RGDP','RINV','RDURINV','RPNRI','RPRI',...
            'IPMAN','RTTLCONS','RPCESV','R_PNRI','R_PRI','RCONS','R_NONDURCO','R_DURCO',...
            'RGOV','REXP','RM','RCPH','ULC','HOURS','Y_L','GDPDEF'}];  % 'RCOFC',
        
% Choice for linear Trends
% -------------------------------------------------------------------------
% Endogenous Liquidity by Bigio (2012):
% DateEndTrend=End_GreatRec+12; % Date for end of linear trend <- Fix here
% initlindate=Beg_GreatRec-1; % Beginning of linear t <end-here
% datenum('31-Sept-1980');
% -------------------------------------------------------------------------
% Banks, Liquidity Management and Monetary Policy by Bianchi and Bigio
% (2014)
% 
% -------------------------------------------------------------------------
%initlindate=Beg_DecPlot; % For linear extrapolation towards end of sample
DateBegTrend=Beg_GreatRec-2;
DateEndTrend=End_GreatRec+8; % For linear extrapolation towards end of sample
DateBegArt  =Beg_GreatRec-2;
DateEndArt  =End_GreatRec+8;

% Bandpass Filter Parameters
pl=6; pu=32;

% Compute Deviations from Trend etc.
BDP_transformations;
...
I1_list= [Alist FernaldName_List rinv_list {'RGDP','RINV','RDURINV','RPNRI','RPRI','R_PNRI','R_PRI',...
    'IPMAN','RPCESV','RTTLCONS','RCOFC','RCONS','R_NONDURCO','R_DURCO','RGOV','REXP','RM','RCPH','ULC','HOURS','Y_L'}];
...
% <- Sergio: generalize here 
aux_list={'RPRI','RPNRI','R_PRI','R_PNRI','RCONS','RNONDURCO','RDURCO','RGOV','REXP'};
TFP_rinvratios;
...
disp(['Labor Wedge Shimer: ' num2str(mean(ShimerLabWedge(ShimerLabWedge>0)))]);
disp(['Labor Wedge EL paper: ' num2str(mean(LabWedge(LabWedge>0)))]);

%% Computation of Demand Shock in Bianchi and Bigio (2014)
% Demand Shock in Bianchi Bigio paper:
% save forJavi_0715.mat;
epsilon_bar=1.8;
...
if plotit==1
    ...
    figure
    % close all;
    pseries={'devhpart_A_fer','devhp_A_fer'};
    % 'ShimerLabWedge',
    date_types='gr';
    FRED_fix_dates;
    BDP_autoplot
...
%    legend('FLabWedge','A_fer');
    ...
    figure
    % close all;
    pseries={'hp_A_fer','hpart_A_fer','A_fer'};
    % 'ShimerLabWedge',
    date_types='all';
    FRED_fix_dates;
    BDP_autoplot
...
%% For Endogenous Liquidity and the Business Cycle
...
% Using Econometrics toolbox
AR1 = arima('ARLags',1);

% Using YULE-Walker
display('Estimates from BEA measured TFP');
aux=devhp_A_BEA(logical(HistIndic));
aux=aux(isfinite(aux));
[ar_coeffs,NoiseVariance,reflect_coeffs] = aryule(aux,1);
mu_NIPA=0;
rho_NIPA=-ar_coeffs(2);
sigma_NIPA=sqrt(NoiseVariance);
EstMdl = estimate(AR1,aux);

display('Estimates from Fernald measured TFP')
...
display('Estimates from Fernald measured utilization based TFP')
...
for ii=1:numel(aux_list)    
    for dd=1:5
        % All the Sample
         ...
         eval(['sd_' char(str) '_' char(aux_list(ii))...
             '= sqrt(mean((' char(aux_list(ii)) '(range)-mu_' char(str) '_' char(aux_list(ii)) ').^2))/mu_' char(str) '_' char(aux_list(ii)) ';']);
    end        
end
    ...
    for ii=1:numel(aux_list)    
        for dd=1:5
            % All the Sample
             ...
             eval(['sqdiff_' char(str) '_devhp_' char(aux_list(ii))          '= (devhp_' char(aux_list(ii)) ').^2;']);
        end        
    end
else
    for ii=1:numel(aux_list)    
        for dd=1:5
            % All the Sample
             ...
             eval(['sqdiff_' char(str) '_devhpart_' char(aux_list(ii))          '= (devhpart_' char(aux_list(ii)) '-mean(devhpart_'...
                 char(aux_list(ii)) '(range))).^2;']);
         end        
    end
...
if artcorr==0
    for ii=1:numel(aux_list)
        for dd=1:5
            ...
            range=logical(range_1.*range_2.*HistIndic);

             % eval(['sd_' char(str) '_devhp_' char(aux_list(ii))          '= sqrt(mean((devhp_' char(aux_list(ii)) '(range)-mu_' char(str) '_devhp_' char(aux_list(ii)) ').^2))/mu_' char(str) '_devhp_' char(aux_list(ii)) ';']);
%              eval(['sigma_' char(str) '_' char(aux_list(ii)) '_Y=sd_' char(str) '_devhp_'  char(aux_list(ii)) '/sd_' char(str) '_devhp_RGDP;']);
%              eval(['se_' char(str) '_' char(aux_list(ii)) '_Y=sigma_' char(str) '_' char(aux_list(ii))...
%                  '_Y^2*sqdiff_' char(str) '_devhp_'  char(aux_list(ii)) '-sqdiff_' char(str) '_devhp_RGDP;']);

             eval(['sigma_' char(str) '_' char(aux_list(ii)) '_Y=sd_' char(str) '_devhp_'  char(aux_list(ii)) '/sd_' char(str) '_devhp_RGDP;']);
             eval(['[sigma2_' char(str) '_'  char(aux_list(ii)) '_Y,sigma_' char(str) '_'  char(aux_list(ii))...
                '_Y_se]=ratsdev_gmm(devhp_' char(aux_list(ii)) '(logical(range)),devhp_RGDP(logical(range)),30,1);']);
             eval(['se_' char(str) '_' char(aux_list(ii)) '_Y=sigma_' char(str) '_' char(aux_list(ii))...
                 '_Y^2*sqdiff_' char(str) '_devhp_'  char(aux_list(ii)) '(range)-sqdiff_' char(str) '_devhp_RGDP(range);']);
             eval(['mse_' char(str) '_' char(aux_list(ii)) '_Y=mean(se_' char(str) '_' char(aux_list(ii))...
                '_Y.^2)^(1/2)/sd_h_devhp_RGDP^2/sqrt(sum(range));']);
%               eval(['se_' char(str) '_' char(aux_list(ii)) '_Y=(sqdiff_' char(str) '_devhp_'  char(aux_list(ii))...
%                   '(range)./sd_' char(str) '_devhp_RGDP^2);']);
             eval(['Val5=prctile(se_' char(str) '_' char(aux_list(ii)) '_Y,10);']);
             eval(['Val95=prctile(se_' char(str) '_' char(aux_list(ii)) '_Y,90);']);
%              eval(['sigma5_' char(str) '_' char(aux_list(ii)) '_Y=Val5/sd_' char(str) '_devhpart_RGDP^2'...
%                  '+sigma_' char(str) '_' char(aux_list(ii)) '_Y^2;']);
%              eval(['sigma95_' char(str) '_' char(aux_list(ii)) '_Y=Val95/sd_' char(str) '_devhpart_RGDP^2'...
%                  '+sigma_' char(str) '_' char(aux_list(ii)) '_Y^2;']);
             eval(['mse_' char(str) '_' char(aux_list(ii)) '_Y=mean(se_' char(str) '_' char(aux_list(ii)) '_Y.^2)^(1/2);']);
             eval(['sigma5_' char(str) '_' char(aux_list(ii)) '_Y=sqrt(Val5);']);
             eval(['sigma95_' char(str) '_' char(aux_list(ii)) '_Y=sqrt(Val95);']);
    ...
    for ii=1:numel(aux_list)    
        ...
        if sum(range)>0
             % eval(['sd_' char(str) '_devhp_' char(aux_list(ii))          '= sqrt(mean((devhp_' char(aux_list(ii)) '(range)-mu_' char(str) '_devhp_' char(aux_list(ii)) ').^2))/mu_' char(str) '_devhp_' char(aux_list(ii)) ';']);
             % eval(['sigma_' char(str) '_' char(aux_list(ii)) '_Y=sqrt(mean(sqdiff_' char(str) '_devhpart_'  char(aux_list(ii))...
               % '(range))/mean(sqdiff_' char(str) '_devhpart_RGDP(range)));']);
             eval(['sigma_' char(str) '_' char(aux_list(ii)) '_Y=sd_' char(str) '_devhpart_'  char(aux_list(ii)) '/sd_' char(str) '_devhpart_RGDP;']);
             eval(['[sigma2_' char(str) '_'  char(aux_list(ii)) '_Y,sigma_' char(str) '_'  char(aux_list(ii))...
                '_Y_se]=ratsdev_gmm(devhpart_RGDP(logical(range)),devhpart_' char(aux_list(ii)) '(logical(range)),12,1);']);
             eval(['se_' char(str) '_' char(aux_list(ii)) '_Y=sigma_' char(str) '_' char(aux_list(ii))...
                 '_Y^2*sqdiff_' char(str) '_devhpart_'  char(aux_list(ii)) '(range)-sqdiff_' char(str) '_devhpart_RGDP(range);']);
             
            eval(['mse_' char(str) '_' char(aux_list(ii)) '_Y=mean(se_' char(str) '_' char(aux_list(ii))...
                '_Y.^2)^(1/2)/sd_h_devhpart_RGDP^2*sqrt(sum(range));']);
%               eval(['se_' char(str) '_' char(aux_list(ii)) '_Y=sqrt(sqdiff_' char(str) '_devhpart_'  char(aux_list(ii))...
%                   '(range)./sd_' char(str) '_devhpart_RGDP^2);']);
             eval(['Val5=prctile(se_' char(str) '_' char(aux_list(ii)) '_Y,5);']);
             eval(['Val95=prctile(se_' char(str) '_' char(aux_list(ii)) '_Y,95);']);
%              eval(['sigma5_' char(str) '_' char(aux_list(ii)) '_Y=Val5/sd_' char(str) '_devhpart_RGDP^2'...
%                  '+sigma_' char(str) '_' char(aux_list(ii)) '_Y^2;']);
%              eval(['sigma95_' char(str) '_' char(aux_list(ii)) '_Y=Val95/sd_' char(str) '_devhpart_RGDP^2'...
%                  '+sigma_' char(str) '_' char(aux_list(ii)) '_Y^2;']);
             eval(['sigma5_' char(str) '_' char(aux_list(ii)) '_Y=Val5;']);
             eval(['sigma95_' char(str) '_' char(aux_list(ii)) '_Y=Val95;']);
             
%              eval(['se_qdiff_' char(str) '_devhpart_' char(aux_list(ii))          '= (devhpart_' char(aux_list(ii)) '(range)-mean(devhpart_' ...             
%                 char(aux_list(ii)) '(range))).^2;']);
            end
         end        
    end
...
if artcorr==0
    for ii=1:numel(aux_list)
        eval(['range_1=isfinite(devhp_' char(aux_list(ii)) ');']);
        eval(['range_2=isfinite(devhp_' char(aux_list(ii)) ');']);
        range=range_1.*range_2.*HistIndic;
        eval(['corr_' char(aux_list(ii)) '=xcorr(devhp_RGDP(logical(range)),devhp_' char(aux_list(ii)) '(logical(range)),maxcorr,''coeff'');']);
        eval(['[corr2_' char(aux_list(ii)) ',corr2_' char(aux_list(ii))...
            '_se]=correl_gmm(devhp_RGDP(logical(range)),devhp_' char(aux_list(ii)) '(logical(range)),12,1);']);
        
        eval(['[R_' char(aux_list(ii)) ',P_' char(aux_list(ii))...
            ',RLO_' char(aux_list(ii)) ',RUP_' char(aux_list(ii))... 
            ']=corrcoef(devhp_RGDP(logical(range)),devhp_' char(aux_list(ii)) '(logical(range)));']);
             
        SD1=std(devhp_RGDP(logical(range)));
        M1=mean(devhp_RGDP(logical(range)));
        eval([ 'SD2=std(devhp_' char(aux_list(ii)) '(logical(range)));']);
        eval([ 'M2=mean(devhp_' char(aux_list(ii)) '(logical(range)));']);
        eval(['aux=((devhp_RGDP(logical(range))-M1).*(devhp_' char(aux_list(ii)) '(logical(range))-M2)/SD1/'...
            'SD2-corr_' char(aux_list(ii))...
            '(maxcorr+1)).^2;']);
        eval(['corr_' char(aux_list(ii))...
            '_se=sqrt(mean(aux(isfinite(aux))));']);
        eval(['corr_' char(aux_list(ii))...
            '_se=sqrt(mean(aux(isfinite(aux))));']);
    ...
    for ii=1:numel(aux_list)
        ...
        if sum(range)>0
            ...
            eval(['[corr2_' char(aux_list(ii)) ',corr2_' char(aux_list(ii))...
                '_se]=correl_gmm(devhpart_RGDP(logical(range)),devhpart_' char(aux_list(ii)) '(logical(range)),12,1);']);

            eval(['[R_' char(aux_list(ii)) ',P_' char(aux_list(ii))...
                ',RLO_' char(aux_list(ii)) ',RUP_' char(aux_list(ii))... 
                ']=corrcoef(devhpart_RGDP(logical(range)),devhpart_' char(aux_list(ii)) '(logical(range)));']);     
            SD1=std(devhpart_RGDP(logical(range)));
            M1=mean(devhpart_RGDP(logical(range)));
            eval([ 'SD2=std(devhpart_' char(aux_list(ii)) '(logical(range)));']);
            eval([ 'M2=mean(devhpart_' char(aux_list(ii)) '(logical(range)));']);
            eval(['aux=((devhpart_RGDP(logical(range))-M1).*(devhpart_' char(aux_list(ii)) '(logical(range))-M2)/SD1/'...
                'SD2-corr_' char(aux_list(ii))...
                '(maxcorr+1)).^2;']);
    %         eval(['corr_' char(aux_list(ii))...
    %             '_se=sqrt(mean(aux(isfinite(aux))))*sqrt(sum(range));']);
    %         eval(['corr_' char(aux_list(ii))...
    %             '_se=sqrt((1-corr_' char(aux_list(ii)) '(maxcorr+1)^2)/(sum(range)-2));']); 
...
for ii=1:RecsInSample
    ...
    % Recession Dates
    ThisRecession = busdate([datenum(Recessions(RecNum,1)), datenum(Recessions(RecNum,1))]);
...
for qq=1:numel(comp_list)
    ...
    eval([char(comp_list(qq)) '_MeanComp=mean(' char(comp_list(qq)) '_comps(1:end-1,:));']);
end
...
display([num2str(corr_A_NIPA(maxcorr+1)) ' (' num2str(corr2_A_NIPA_se) ')']);
display('Contemporaneous TFP Correlation with Hours');
display(num2str(corrh_A_NIPA(maxcorr+1)));
display('Contemporaneous TFP Correlation with Investment');
display(num2str(corri_A_NIPA(maxcorr+1)));
display('***********************************');
display('Correlations with HP deviations (BEA)');
display('***********************************');
display('Contemporaneous TFP Correlation with Output');
display([num2str(corr_A_BEA(maxcorr+1))  ' (' num2str(corr2_A_BEA_se) ')']);
display('Contemporaneous TFP Correlation with Hours');
display([num2str(corrh_A_BEA(maxcorr+1))]);
display('Contemporaneous TFP Correlation with Investment');
display([num2str(corri_A_BEA(maxcorr+1))]);
display('***********************************');
display('Correlations with HP deviations (FERNALD)');
display('***********************************');
display('Contemporaneous TFP Correlation with Output');
display([num2str(corr_AU_fer(maxcorr+1)) ' (' num2str(corr2_AU_fer_se) ')']);
display('Contemporaneous TFP Correlation with Hours');
display([num2str(corrh_AU_fer(maxcorr+1))]);
display('Contemporaneous TFP Correlation with Investment');
display([num2str(corri_AU_fer(maxcorr+1))]);
display('Contemporaneous TFP Correlation with Durables + Investment');
display([num2str(corrz_AU_fer(maxcorr+1))]);
display('***********************************');
display('Correlations with HP deviations');
display('***********************************');
display('Contemporaneous HOUR Correlation with Output');
display([num2str(corr_HOURS(maxcorr+1)) ' (' num2str(corr2_HOURS_se) ')']);
display('Contemporaneous INVESTMENT Correlation with Output');
display([num2str(corr_RINV(maxcorr+1)) ' (' num2str(corr2_RINV_se) ')']);
display('Contemporaneous Private Non-Residential INVESTMENT Correlation with Output');
display([num2str(corr_RPNRI(maxcorr+1)) ' (' num2str(corr2_RPNRI_se) ')']);
display('Contemporaneous DURABLES+INVESTMENT Correlation with Output');
display([num2str(corr_RDURINV(maxcorr+1)) ' (' num2str(corr2_RDURINV_se) ')']);
% display('Contemporaneous Non-DURABLE Consumption with Output');
% display([num2str(corr_R_NONDURCO(maxcorr+1)) ' (' num2str(corr_NONDURCO_sd) ')']);
display('Contemporaneous Real Consumption');
display([num2str(corr_RCONS(maxcorr+1)) ' (' num2str(corr2_RCONS_se) ')']);
display('Contemporaneous Labor Productivity Correlation');
display([num2str(corr_Y_L(maxcorr+1)) ' (' num2str(corr2_Y_L_se) ')']);
display('Contemporaneous TFP Correlation with Output');
display([num2str(corr_A_fer(maxcorr+1)) ' (' num2str(corr2_A_fer_se) ')']);
display('***********************************');
display('Correlations with HP deviations');
display('***********************************');
display('Contemporaneous HOUR Correlation with Output');
display([num2str(corr2_HOURS) ' (' num2str(corr2_HOURS_se) ')']);
display('Contemporaneous INVESTMENT Correlation with Output');
display([num2str(corr2_RINV) ' (' num2str(corr2_RINV_se) ')']);
display('Contemporaneous Private Non-Residential INVESTMENT Correlation with Output');
display([num2str(corr2_RPNRI) ' (' num2str(corr2_RPNRI_se) ')']);
display('Contemporaneous DURABLES+INVESTMENT Correlation with Output');
display([num2str(corr2_RDURINV) ' (' num2str(corr2_RDURINV_se) ')']);
% display('Contemporaneous Non-DURABLE Consumption with Output');
% display([num2str(corr_R_NONDURCO(maxcorr+1)) ' (' num2str(corr_NONDURCO_sd) ')']);
display('Contemporaneous Real Consumption');
display([num2str(corr2_RCONS) ' (' num2str(corr2_RCONS_se) ')']);
display('Contemporaneous Labor Productivity Correlation');
display([num2str(corr2_Y_L) ' (' num2str(corr2_Y_L_se) ')']);
display('Contemporaneous TFP Correlation with Output');
display([num2str(corr2_A_fer) ' (' num2str(corr2_A_fer_se) ')']);
display('***********************************');
...
display('***********************************');
display('Hours:     ');
display([num2str(sigma2_h_HOURS_Y) ' (' num2str(sigma5_h_HOURS_Y) ',' num2str(sigma95_h_HOURS_Y) ') (' num2str(sigma_h_HOURS_Y_se) ')']);
display('Investment:');
display([num2str(sigma2_h_RINV_Y) ' (' num2str(sigma5_h_RINV_Y) ',' num2str(sigma95_h_RINV_Y)  ') (' num2str(sigma_h_RINV_Y_se) ')']);
display('Private Non-Residential Investment:');
display([num2str(sigma2_h_RPNRI_Y) ' (' num2str(sigma5_h_RPNRI_Y) ',' num2str(sigma95_h_RPNRI_Y)  ') (' num2str(sigma_h_RPNRI_Y_se) ')']);
display('Durables +Investment:');
display([num2str(sigma2_h_RDURINV_Y) ' (' num2str(sigma5_h_RDURINV_Y) ',' num2str(sigma95_h_RDURINV_Y)  ') (' num2str(sigma_h_RDURINV_Y_se) ')']);
display('Consumption:');
display([num2str(sigma2_h_RCONS_Y) ' (' num2str(sigma5_h_RCONS_Y) ',' num2str(sigma95_h_RCONS_Y)  ') (' num2str(sigma_h_RCONS_Y_se) ')']);
display('Non-Durco Consumption:');
display(num2str(sigma2_h_R_NONDURCO_Y));
display('A FER:');
display([num2str(sigma2_h_A_fer_Y) ' (' num2str(sigma5_h_A_fer_Y) ',' num2str(sigma95_h_A_fer_Y)  ') (' num2str(sigma_h_A_fer_Y_se) ')']);
display('AU FERNALD:');
display([num2str(sigma2_h_AU_fer_Y) ' (' num2str(sigma5_h_AU_fer_Y) ',' num2str(sigma95_h_AU_fer_Y)  ') (' num2str(sigma_h_AU_fer_Y_se) ')']);
display('Labor Productivity:');
display([num2str(sigma2_h_Y_L_Y) ' (' num2str(sigma5_h_Y_L_Y) ',' num2str(sigma95_h_Y_L_Y)  ') (' num2str(sigma_h_Y_L_Y_se) ')']);
display('***********************************');
...
fprintf(fid,'\n $Y_{t}$          & 1     & %0.3g (%0.3g) & %0.3g (%0.3g) & %0.3g (%0.3g) & %0.3g (%0.3g) \\\\ \\hline',corr2_A_fer,corr2_A_fer_se,corr2_HOURS,corr2_HOURS_se,...
    corr2_RCONS,corr2_RCONS_se,corr2_RINV,corr2_RINV_se)               ;
...
fprintf(fid,'\n & 1 & %0.3g (%0.3g) & %0.3g (%0.3g) & %0.3g (%0.3g) & %0.3g (%0.3g) \\\\ \\hline',...
    sigma_h_A_fer_Y,sigma_h_A_fer_Y_se,sigma_h_HOURS_Y,sigma_h_HOURS_Y_se,...
    sigma_h_RCONS_Y,sigma_h_RCONS_Y_se,sigma_h_RINV_Y,sigma_h_RINV_Y_se)               ;
fclose('all');
